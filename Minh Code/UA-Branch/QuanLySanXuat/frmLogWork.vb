...
Public Class frmLogWork
    ...
    Private Sub AddNewRecord()
        Try
            Dim qry As String = "Insert into tbllogwork (TDate, Employee, FishType, JobType, KCS, Quantity, TotalWeight, FishPartition, RatePerPack, TotalAmount, Note, CreatedDate, UpdatedDate, CreatedBy, UpdatedBy) Values ('" & Format(DateTimePicker1.Value, "yyyy/MM/dd") & "'," & cmbEmployee.SelectedValue & "," & cmbFishType.SelectedValue & "," & cmbJobType.SelectedValue & "," & IIf(cmbKCS.SelectedValue = Nothing, "NULL", cmbKCS.SelectedValue) & ", " & GetFloatValue(txtQuantity.Text) & "," & IIf(txtTotalWeight.Text="", 0, txtTotalWeight.Text.Replace(",", ".")) & "," & IIf(txtFishPartition.Text = "", 0, txtFishPartition.Text.Replace(",", ".")) & "," & txtRateperPack.Text.Replace(",", "").Replace(".", "") & "," & txtTotalAmount.Text.Replace(",", "").Replace(".", "") & ",'" & txtNote.Text + "','" & DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss") & "','" & DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss") & "'," & usersDt.Rows(0)("ID") & "," & usersDt.Rows(0)("ID") & ")"
    ...
    Public Sub CalculateTotalWeight()
        Try
            ...
            txtTotalWeight.Text = Math.Round(((ValNewF(txtQuantity.Text) * txtRateperPack.Tag) + iif(txtFishPartition.Text="", 0, txtFishPartition.Text)), 2)
